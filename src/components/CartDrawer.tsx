...
export function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <aside
        className={`fixed right-0 top-0 z-50 flex h-full w-full max-w-[100vw] sm:max-w-md md:max-w-lg flex-col bg-herb-white text-charcoal shadow-2xl transition-transform duration-300 ${
          isOpen ? "translate-x-0" : "translate-x-full"
        }`}
        aria-hidden={!isOpen}
      >
        {/* Drawer Header */}
        <div className="flex items-center justify-between border-b border-line px-4 sm:px-6 py-4 sm:py-5 bg-white">
          <div>
            <h2 className="font-display text-xl sm:text-2xl text-charcoal flex items-center gap-2">
              <ShoppingBagIcon className="h-6 w-6 text-zest-deep shrink-0" />
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-bone text-charcoal/70 hover:bg-basil hover:text-bone transition-colors cursor-pointer shrink-0"
            >
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 min-h-0">
          {lines.length === 0 ? (
            ...
            <ul className="divide-y divide-line">
              {lines.map((line) => (
                <li key={line.id} className="py-4 flex gap-3 sm:gap-4 items-center">
                  {line.image && (
                    <div className="relative h-14 w-14 sm:h-16 sm:w-16 shrink-0 overflow-hidden rounded-xl bg-bone border border-line">
                      <Image
                        src={line.image}
                        alt={line.name}
                        fill
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        ...
                        className="text-xs text-charcoal/40 hover:text-red-500 p-1 cursor-pointer shrink-0"
                        title="Remove item"
                      >
